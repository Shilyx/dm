...
type DmArray struct {
	TypeData
	m_arrDesc *ArrayDescriptor // 数组的描述信?

	m_arrData []TypeData // 数组中各行数据?

	m_objArray interface{} // 从服务端获取?

	m_itemCount int // 本次获取的行?

	m_itemSize int // 数组中一个数组项的大小，单位bytes

	m_objCount int // 一个数组项中存在对象类型的个数（class、动态数?

	m_strCount int // 一个数组项中存在字符串类型的个?
...
// 数据库自定义数组Array构造函数，typeName为库中定义的数组类型名称，elements为该数组类型的每个?//
// 例如，自定义数组类型语句为：create or replace type myArray is array int[];
//
// 则绑入绑出的go对象?
// val := dm.NewDmArray("myArray", []interface{} {123, 456})
...
// 获取Array对象的指定偏移和执行长度go数据对象 index?开?
