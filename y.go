 ...
 *
 * DB 连接顺序? * 1. well distribution，每次连接都从列表的下一个节点开? * 2. 用DB sort值按从大到小排序，sort为一个四位数XXXX，个?-serverStatus，十?-serverMode，共 有三种模式，最优先?*100, 次优先的*10
 */
type epGroup struct {
	name       string
	epList     []*ep
...
func (g *epGroup) connect(connector *DmConnector) (*DmConnection, error) {
	...
	var ex error = nil
	// 如果配置了loginMode的主、备等优先策略，而未找到最高优先级的节点时持续循环switchtimes次，如果最终还是没有找到最高优先级则选择次优先级?	// 如果只有一个节点，一轮即可决定是否连接；多个节点时保证switchTimes轮尝试，最后一轮决定用哪个节点（由于节点已经按照模式优先级排序，最后一轮理论上就是连第一个节点）
...
*
* @exception
* DBError.ECJDBC_INVALID_SERVER_MODE 有站点的模式不匹?* DBError.ECJDBC_COMMUNITION_ERROR 所有站点都连不? */
